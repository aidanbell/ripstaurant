...
import type { City, ClosureRow } from "../src";
import { TORONTO_FIXTURE } from "../fixtures/toronto";
import type { FixtureLocation } from "../fixtures/toronto";
...
const TORONTO: City = {
  ...
  bbox: [-79.6393, 43.581, -79.1153, 43.8555],
};
...
function toDetail(f: FixtureLocation): LocationDetail {
  ...
  const ref = (key: string) => {
    if (!(key in f.sources))
      throw new Error(`${f.id}: unknown source "${key}"`);
...
function syntheticRows(n: number): ClosureRow[] {
  ...
  return Array.from({ length: n }, (_, i) => {
    ...
    const row: ClosureRow = {
      ...
      type:
        rand() < 0.85 ? pick(DEFAULT_VISIBLE_TYPES) : pick(EventType.options),
      status: "closed",
      ...
      rs:
        rand() < 0.6
          ? []
          : [pick(ReasonCode.options.filter((c) => c !== "unknown"))],
    };
...
/** JSON formatted with the repo's Prettier config, so regenerated files pass `format:check`. */
async function toJson(data: unknown, path: string): Promise<string> {
  const options = await resolveConfig(path);
  return format(JSON.stringify(data), { ...options, filepath: path });
}

async function write(dir: string, rel: string, data: unknown) {
  const path = `${dir}/${rel}`;
  await mkdir(path.slice(0, path.lastIndexOf("/")), { recursive: true });
  await Bun.write(path, await toJson(data, path));
}
...
async function build(
  ...
  // The hash is of the list's content, so it only changes when the data does.
  const listRel = listPath(TORONTO.slug, hash(JSON.stringify(list)));
  await write(outDir, listRel, list);

  for (const d of details)
    await write(outDir, locationPath(TORONTO.slug, d.id), d);
  ...
  console.log(
    `  ${list.closures.length} closures, ${details.length} location files`,
  );
  console.log(
    `  ${listRel}: ${(minified.length / 1024).toFixed(0)} KB minified, ${(gzipSync(minified).length / 1024).toFixed(0)} KB gzipped`,
  );
}
...
if (syntheticArg > 0) {
  ...
  const rows = details.flatMap((d) =>
    rowsFromLocation(d, TORONTO_NEIGHBOURHOODS),
  );
  ...
  for (const d of details)
    for (const o of d.occupants)
      if (
        o.event?.next?.loc &&
        !details.some((x) => x.id === o.event!.next!.loc)
      )
        throw new Error(
          `${d.id}: next.loc ${o.event.next.loc} has no detail file`,
        );
