...
const details = await Promise.all(
  locationFiles.map(async (f) =>
    LocationDetail.parse(await read(`${city.slug}/locations/${f}`)),
  ),
);
...
describe("committed sample", () => {
  ...
  test("detail files are named by location id", () => {
    for (const d of details)
      expect(locationFiles).toContain(
        locationPath(city.slug, d.id).split("/").pop()!,
      );
  });
  ...
  test("covers the tricky cases the frontend must handle", () => {
    ...
    expect(new Set(list.closures.map((r) => r.loc)).size).toBeLessThan(
      list.closures.length,
    ); // shared location
  });
});
...
describe("schema guards", () => {
  ...
  const withRow = (patch: Partial<ClosureRow>) => ({
    ...list,
    closures: [{ ...row, ...patch }],
  });
  ...
  test("rejects a neighbourhood index out of range", () => {
    expect(
      CityList.safeParse(withRow({ hood: list.hoods.length })).success,
    ).toBe(false);
  });

  test("rejects coordinates outside the city", () => {
    expect(
      CityList.safeParse(withRow({ ll: [-73.5673, 45.5017] })).success,
    ).toBe(false);
  });

  test("rejects swapped lat/lng", () => {
    expect(
      CityList.safeParse(withRow({ ll: [row.ll[1], row.ll[0]] })).success,
    ).toBe(false);
  });

  test("rejects a date range that ends before it starts", () => {
    expect(
      CityList.safeParse(withRow({ d: ["2025-05-01", "2025-04-01"] })).success,
    ).toBe(false);
  });

  test("rejects a closed event with no dates", () => {
    expect(CityList.safeParse(withRow({ d: [null, null] })).success).toBe(
      false,
    );
  });
  ...
  test("rejects events before the coverage window", () => {
    expect(
      CityList.safeParse(withRow({ d: ["2021-06-01", "2021-06-01"] })).success,
    ).toBe(false);
  });

  test("rejects non-food businesses and retracted events", () => {
    expect(
      CityList.safeParse(withRow({ bt: "non_food" as never })).success,
    ).toBe(false);
    expect(
      CityList.safeParse(withRow({ status: "retracted" as never })).success,
    ).toBe(false);
  });

  test("rejects duplicate event ids", () => {
    expect(CityList.safeParse({ ...list, closures: [row, row] }).success).toBe(
      false,
    );
  });
